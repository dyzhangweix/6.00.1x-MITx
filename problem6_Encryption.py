...
def buildCoder(shift):
    ...
    for i in range (26-shift):
        coder[upper[i]]=upper[i+shift]
        coder[lower[i]]=lower[i+shift]
...
def applyCoder(text, coder):
    ...
    afterCoder=''
    for e in text:
        if e not in string.lowercase and e not in string.uppercase:
            afterCoder+=e
        else: 
            afterCoder+=coder[e]
    return afterCoder
...
def findBestShift(wordList, text):
    ...
    for shift in range(26):
        numOfWords=0
        tempText=applyShift(text, shift)
        words = re.sub('['+string.punctuation+']', " ",  tempText).split()
        for word in words:
