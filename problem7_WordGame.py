...
def getWordScore(word, n):
    ...
    sumScore=0
    for char in word:
        sumScore+=SCRABBLE_LETTER_VALUES[char]
    sumScore*=len(word)
    if len(word)==n:
        sumScore+=50
    return sumScore
...
def isValidWord(word, hand, wordList):
    ...
    def isInHand(word, hand):
        ...
        i=0
        temp=True
        while temp==True and i<len(word):
            if handCopy.get(word[i], -1)<0:
                temp=False
                break
            handCopy[word[i]]-=1
    ...
    return len(word)>0 and isInHand(word, hand) and word in wordList
